    ...
    return (
        <div className={"container col-10 text-dark border border-dark rounded-5 mt-4 text-center"}>
            ...
            {allClients.forEach(x=> {
                clients.push((<tr><td className={"align-middle"}>{x.name} {x.surname}</td><td className={"align-middle"}>{x.email}</td><td className={"align-middle"}>{x.rating.rating} {element}</td><td className={"align-middle"}><Link onClick={() => props.onView(x.email,x.role)} to={`/visitProfile/${x.email}/${x.role}`} className={"btn btn-block col-12 btn-primary rounded-5 mb-2"}>Visit Profile</Link></td></tr>));
            })}
